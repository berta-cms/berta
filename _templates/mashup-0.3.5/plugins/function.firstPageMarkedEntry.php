 ...
 * Smarty plugin
 * -------------------------------------------------------------
 * File:     function.firstPageMarkedEntry.php
 * Type:     function
 * Name:     firstPageMarkedEntry
 * Purpose:
 * -------------------------------------------------------------
 */
function smarty_function_firstPageMarkedEntry($params, &$smarty)
{
    global $berta;
    ...
    if (!empty($params['entry'])) {
        ...
        if ($imgs) {
            ...
            list($html, $w, $h) = BertaGallery::getHTML([0 => $img], $params['entry']['__raw']['mediafolder']['value'], 'slideshow', false, true, $sizeRatio);

            $link = '';
            if ($berta->environment != 'engine' && $img['@attributes']['type'] == 'image') {
                $tagKeys = array_keys($params['entry']['tags']);

                $link = smarty_function_bertaLink([
            ...
            $pos = !empty($entry['positionXY']) ? explode(',', $entry['positionXY']) : [rand(0, $viewportWidth - $w), rand(0, $viewportHeight - $h)];
            $entryClasses = smarty_function_entryClasses(['entry' => $params['entry']], $smarty);
