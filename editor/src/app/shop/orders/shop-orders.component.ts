...
@Component({
    ...
    <div *ngFor="let order of orders$ | async" class="setting">
      ...
      <p *ngIf="order.company_reg_no">
        Company reg. no. <span>{{ order.company_reg_no }}</span>
      </p>
      <p *ngIf="order.legal_address">
      ...
      >
        Shipping details
        <span>
          <span *ngIf="order.ship_name"> {{ order.ship_name }}<br /> </span>
          <span *ngIf="order.ship_phone"> {{ order.ship_phone }}<br /> </span>
          <span *ngIf="order.ship_address">
            {{ order.ship_address }}<br />
          </span>
          <span *ngIf="order.ship_email"> {{ order.ship_email }}<br /> </span>
          <span *ngIf="order.ship_notes"> {{ order.ship_notes }}<br /> </span>
        </span>
      </p>
      <p *ngIf="order.send_news">Send news <span>Yes</span></p>
      <p *ngIf="order.promo_code">
