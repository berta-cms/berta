      ...
      <div class="ai-panel" [class.ai-panel--minimized]="isMinimized">
        ...
        >
          ...
          <div class="ai-panel-actions">
            ...
            <button
              (click)="
                $event.stopPropagation(); isMinimized ? restore() : minimize()
              "
              class="minimize-btn"
        ...
        @if (!isMinimized) {
          <div class="ai-messages" #messagesContainer>
            @if ((messages$ | async)?.length === 0) {
              <div class="ai-empty">
                <p>
                  Ask me to help with your content, design, or settings.<br />e.g.
                </p>
            ...
            <div class="ai-actions-row">
              <button
                ...
                [disabled]="
                  !inputText.trim() ||
                  (isLoading$ | async) ||
                  !!(dailyLimitMessage$ | async)
                "
              >
                Send
              </button>
              @if (dailyUsage$ | async; as usage) {
                <span class="ai-usage-counter">{{ usage.count }} / {{ usage.limit }} daily limit</span>
              }
            </div>
          </div>
        }
...
export class AiAssistantComponent implements AfterViewChecked, OnDestroy {
  ...
  dailyUsage$: Observable<{ count: number; limit: number } | null>;
  ...
  constructor(private store: Store) {
    ...
    this.dailyLimitMessage$ = this.store.select(
      AiAssistantState.dailyLimitMessage,
    );
    this.dailyUsage$ = this.store.select(AiAssistantState.dailyUsage);
    this.hasFeedback$ = this.store
      .select(AppState.getAiFeedbackUrl)
      .pipe(map((url) => !!url));
    this.subs.push(
