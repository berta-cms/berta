export function replaceContent(dom: Document, sectionId: string, sectionHtml: string): void {
  const element = dom.getElementById(sectionId)
  element.innerHTML = ''
  element.appendChild(dom.createRange().createContextualFragment(sectionHtml))
}

export function removeExtraAddBtnAndAddListeners(iframe: HTMLIFrameElement): void {
