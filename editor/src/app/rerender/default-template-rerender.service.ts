...
export class DefaultTemplateRerenderService extends TemplateRerenderService {
  ...
  handle(iframe: HTMLIFrameElement) {
    ...
    const navigationSubscr = this.actions$
      .pipe(ofActionDispatched(UpdateNavigationSiteSettingsAction))
      .subscribe((action: UpdateNavigationSiteSettingsAction) => {
        ...
        switch (setting) {
          ...
          case 'landingSectionPageHeadingVisible':
            ...
            if (action.payload.landingSectionPageHeadingVisible === 'no')
              return;
